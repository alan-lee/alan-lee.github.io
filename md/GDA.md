高斯辨别分析是一种生成学习算法。在该模型中，假定对于给定的目标$$y$$的值，$$x$$服从多元高斯分布。根据训练集合中的样本数据来拟合参数。再将新样本的数据带入到模型中分别计算样本属于各个分类的概率，并概率最大的作为其所属的类别。
...
其中，$$\mu$$为多元高斯分布的期望，$$\Sigma$$为数据集中$$X$$的协方差，$$|\Sigma|$$表示为协方差的行列式，是一个标量。若$$X$$为$$m\times n$$的矩阵，则$$\mu \in R^{n}, \Sigma \in R^{n \times n}$$
...
```
```math
p(x|y=0; \mu_{0}, \Sigma) = \frac{1}{(2\pi)^{\frac{n}{2}}|\Sigma|^{\frac{1}{2}}}\exp(-\frac{1}{2}(x-\mu_{0})^{T}\Sigma^{-1}(x-\mu_{0}))
```
```math
p(x|y=1; \mu_{1}, \Sigma) = \frac{1}{(2\pi)^{\frac{n}{2}}|\Sigma|^{\frac{1}{2}}}\exp(-\frac{1}{2}(x-\mu_{1})^{T}\Sigma^{-1}(x-\mu_{1}))
```
...
\phi = \frac{I(y^{(i)} = 1)}{m}
 ```
 ```math
\mu_{0} = \frac{\sum_{i=1}^{m}I(y^{(i)} = 0)x^{(i)}}{\sum_{i=1}^{m}I(y^{(i)} = 0)}
 ```
 ```math
\mu_{1} = \frac{\sum_{i=1}^{m}I(y^{(i)} = 1)x^{(i)}}{\sum_{i=1}^{m}I(y^{(i)} = 1)}
 ```
 ```math
